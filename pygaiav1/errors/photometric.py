...
from pygaiav1.errors.utils import calcZ, calcZBpRp
...
def gMagnitudeErrorEoM(G, nobs=70, extension=0.0):
    """
    ...
    return sqrt((power(gMagnitudeError(G) / _scienceMargin, 2) +
                 _eomCalibrationFloorG * _eomCalibrationFloorG) / nobs_scaled) * _scienceMargin
...
def bpMagnitudeErrorEoM(G, vmini, nobs=70, extension=0.0):
    """
    ...
    return sqrt((power(bpMagnitudeError(G, vmini) / _scienceMargin, 2) +
                 _eomCalibrationFloorBP * _eomCalibrationFloorBP) / nobs_scaled) * _scienceMargin
...
def rpMagnitudeErrorEoM(G, vmini, nobs=70, extension=0.0):
    """
    ...
    return sqrt((power(rpMagnitudeError(G, vmini) / _scienceMargin, 2) +
                 _eomCalibrationFloorRP * _eomCalibrationFloorRP) / nobs_scaled) * _scienceMargin
